...
static int set_crtc_parameters(int fd, struct modeset_dev *dev);
			     ...
			     struct modeset_dev *dev)
{
	...
	if (ret) {
        ...
        ret = set_crtc_parameters(fd, dev);
        if (ret) {
		fprintf(stderr, "unable to setup crtc\n");
		return ret;
	}
...
static int set_crtc_parameters(int fd, struct modeset_dev *dev)
{
    ...
    int propi; /* property index */
    props = drmModeObjectGetProperties(fd, dev->crtc, DRM_MODE_OBJECT_CRTC);

    if (props == NULL) {
        fprintf(stderr, "cannot get properties\n");
        return -ENOENT;
    }

    fprintf(stderr,"found %d properties\n", props->count_props);
    for (propi = 0; propi < props->count_props; propi++) {
        prop = drmModeGetProperty(fd, props->props[propi]);
        fprintf(stderr,"property %s:%d\n", prop->name, prop->prop_id);

        if (strcmp(prop->name,"underscan") == 0) {
            drmModeObjectSetProperty(fd, dev->crtc, DRM_MODE_OBJECT_CRTC, prop->prop_id, 3); /* set overscan to crop */
        } else if (strcmp(prop->name,"underscan hborder") == 0) {
            drmModeObjectSetProperty(fd, dev->crtc, DRM_MODE_OBJECT_CRTC, prop->prop_id, 128); /* set overscan horizontal to 128 */
        } else if (strcmp(prop->name,"underscan vborder") == 0) {
            drmModeObjectSetProperty(fd, dev->crtc, DRM_MODE_OBJECT_CRTC, prop->prop_id, 128); /* set overscan vertical to 128 */
        } 
